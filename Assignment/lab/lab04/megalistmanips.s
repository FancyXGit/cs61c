...
mapLoop:
    # ERROR1: add t1, s0, x0 get the address of node
    # should be lw t1, 0(x0) to get inside the node and get arr address
    lw t1, 0(s0)      # load the address of the array of current node into t1
    lw t2, 4(s0)        # load the size of the node's array into t2

    # ERROR2: type int is 4 bytes wide !!!
    slli t3, t0, 2      # t0 = t0 * 4 caculate offset
    # ERROR3: should not change array address t0
    ...
    # ERROR4: not la but lw; read inside current node to get the next node
    lw a0, 8(s0)        # load the address of the next node into a0
    # ERROR5: dont load. s1 is already the address
