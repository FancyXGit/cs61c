...
#pragma omp parallel
  {
    ...
    for (int i = curr_thread; i < ARRAY_SIZE; i += thread_num)
    {
      z[i] = x[i] + y[i];
    }
  }
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    {
      for (int i = curr_thread * block_size; i < (curr_thread + 1) * block_size; i++)
      {
        z[i] = x[i] + y[i];
      }
    }
    ...
    {
      for (int i = curr_thread * block_size; i < ARRAY_SIZE; i++)
      {
        z[i] = x[i] + y[i];
      }
    }
